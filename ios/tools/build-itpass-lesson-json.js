#!/usr/bin/env node
// 抽取 itpass 课程每个章节/小节的真实学习内容（与 sg 同样 schema）
const fs = require('fs');
const path = require('path');
const vm = require('vm');

const ROOT = path.resolve(__dirname, '../..');
const SRC = path.join(ROOT, 'source/miniprogram-pilot/packages/course-itpass/data');
const OUT_DIR = path.join(ROOT, 'ios/CoCoiOS/Resources/CourseData');
const OUT = path.join(OUT_DIR, 'itpass-lesson-detail.json');
...
function loadInVM(filePath) {
    ...
    try {
        ...
        console.error(`[itpass-lesson] ${filePath}: vm error ${e.message}`);
        return null;
    }
...
console.log(`[itpass-lesson] found ${chapterFiles.length} chapter files`);
...
for (const f of chapterFiles) {
    ...
    try {
        ...
        console.error(`[itpass-lesson] ${f} vm error: ${e.message}`);
        continue;
    }
    ...
    console.log(`[itpass-lesson] ${chapterId}: ${units.length} units`);
}
...
console.log(`[itpass-lesson] wrote ${OUT} (${(fs.statSync(OUT).size / 1024).toFixed(1)} KB, ${out.totalUnits} units)`);
